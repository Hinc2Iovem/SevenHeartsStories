...
import { CombinedTranslatedAndNonTranslatedCharacterTypes } from "../../Filters/AboutCharacter/FiltersEverythingCharacterForCharacterRecent";
...
export default function DisplayTranslatedNonTranslatedCharacterRecent({
  ...
  useEffect(() => {
    if (translated?.characterType) {
      setCharacterType(translated.characterType);
    } else if (nonTranslated?.characterType) {
      setCharacterType(nonTranslated.characterType);
    }
